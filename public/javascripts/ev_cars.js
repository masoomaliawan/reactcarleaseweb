...
$('#contactModal').on('show.bs.modal', function (event) {
    ...
    $("#contactSubmit").click(() => {
        ...
        const cardnum=$("#usercard").val();
        const cardcvc=$("#usercvc").val();
        const carddate=$("#userdate").val();

        try {
            $.post("/customer", { username: name, useremail: email, userphone: phone, usercard: cardnum, usercvc: cardcvc, userdate: carddate }, function (data) {
                console.log("done");
            })
        }
        catch (err) {
